...
export default function SavedLineupsScreen({ navigation }) {
  ...
  const renderLineup = useCallback(
    ({ item }) => {
      ...
      return (
        <Card style={styles.lineupCard}>
          <TouchableOpacity
            style={styles.lineupContent}
            onPress={() => navigation.navigate('LineupBuilder', { lineupId: item.id })}
            activeOpacity={0.7}
          >
            <View style={styles.miniCourt}>
              <View style={styles.miniCourtBg}>
                <View style={styles.miniCenterLine} />
                <View style={styles.miniCenterCircle} />
                {item.players?.map((lp) => {
                  const fPos = formation?.positions.find((p) => p.position === lp.position);
                  return fPos ? (
                    <View
                      key={lp.position}
                      style={[styles.miniDot, { left: `${fPos.x * 100}%`, top: `${fPos.y * 100}%` }]}
                    />
                  ) : null;
                })}
              </View>
            ...
            <View style={styles.lineupInfo}>
              ...
              <Text style={[styles.lineupCount, playerCount === 5 && styles.lineupCountComplete]}>
                {playerCount}/5 jogadores
              </Text>
            </View>
          </TouchableOpacity>
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <TouchableOpacity
            style={styles.addBtn}
            onPress={() => navigation.navigate('LineupBuilder')}
          >
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              title="Nenhuma escalação salva"
              description="Crie sua primeira escalação."
              actionLabel="+ Nova escalação"
              onAction={() => navigation.navigate('LineupBuilder')}
            />
          }
        />
      </View>
