...
export default function PlayersScreen({ navigation }) {
  ...
  const filteredPlayers = useMemo(() => sortPlayers(searchPlayers(players, searchQuery), sortBy), [players, searchQuery, sortBy]);
  const renderItem = useCallback(({ item }) => (
    <View style={styles.listItem}>
      <PlayerListItem player={item} onPress={() => navigation.navigate('PlayerDetails', { playerId: item.id })} />
    </View>
  ), [navigation]);
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('PlayerForm', {})} activeOpacity={0.7}>
        ...
        <View style={styles.sortRow}>
          <TouchableOpacity style={styles.sortBtn} onPress={() => setSortMenuVisible(!sortMenuVisible)}>
            <ArrowUpDown size={14} color={colors.textSecondary} />
            <Text style={styles.sortLabel}>{SORT_OPTIONS.find((s) => s.key === sortBy)?.label || 'Nome'}</Text>
          </TouchableOpacity>
        </View>
        {sortMenuVisible && (
          <View style={styles.sortMenu}>
            {SORT_OPTIONS.map((opt) => (
              <TouchableOpacity key={opt.key} style={[styles.sortOption, sortBy === opt.key && styles.sortOptionActive]}
                onPress={() => { setSortBy(opt.key); setSortMenuVisible(false); }}>
                <Text style={[styles.sortOptionText, sortBy === opt.key && styles.sortOptionTextActive]}>{opt.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        )}
        <FlatList data={filteredPlayers} keyExtractor={(item) => item.id} renderItem={renderItem}
          contentContainerStyle={styles.list} ItemSeparatorComponent={() => <View style={{ height: spacing.sm }} />}
          ListEmptyComponent={
            <EmptyState icon={<Users size={40} color={colors.textMuted} />}
...
const styles = StyleSheet.create({
  ...
  list: { paddingHorizontal: spacing.xl, paddingBottom: spacing.xxxl * 2 }, listItem: {},
});
